...
pub mod solution_dfs_follow_up {
    ...
    /// 在新node更新时当
    /// 
    /// - `cur_count == max_count`：表示是不同的值出现数量一样，加入res中。由于
    /// 题目定义相等值必然会连续出现在子树中，一旦是不同值则会中断cur_count
    /// - `cur_count > max_count`：表示之前的res不正确，当前val出现更多
    /// 
    /// ## 注意
    /// 
    ...
    /// ## 复杂度
    /// 
    /// - 时间：O(N)。如果树结点val在后面出现一次多一次，导致res.clear()频繁更新，用时更多
    /// - 空间：O(log N)
    pub struct Solution;
    ...
    impl Solution {
        pub fn find_mode(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
            let mut res = vec![];
            Self::inorder(root, &mut None, &mut 1, &mut 0, &mut res);
            res
        }
...
mod tests {
    ...
    fn basic_solution_dfs_follow_up() {
        ...
        assert_eq!(vec![2], Solution::find_mode(convert_tree("[1,null,2,2]")));
        assert!(is_included_vec(&vec![1,2], &Solution::find_mode(convert_tree("[1,2]"))));
        assert_eq!(vec![1], Solution::find_mode(convert_tree("[1,1]")));
