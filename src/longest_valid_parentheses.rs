...
pub mod solution_dp {
    ...
    ///     *这里不要求对应i-1位置是有效的，由于`dp[i-1]`可以不是有效的？如果i-1不是有效子括号，则
    ///     `dp[i-1]=0, i-dp[i-1]-1=i-1`，而i-1就是`)`无法与i `)`形成括号*。
    ///     如果i位置是有效括号，可能会与之前`i-dp[i-1]-2`形成连续括号`dp[i]=dp[i-1]+2+dp[i-dp[i-1]-2]`
    ...
    ///
    /// 下面这个使用了dp.len=s.len+1，出现了下标问题`assert_eq!(f("()(())".to_string()), 6);`
    ///
    /// ```no_run
    /// // dp[i] = dp[i - 2] + 2 if s[i-1]==')' && s[i-2]=='('
    /// // dp[i] = dp[i-dp[i-1]-2] + dp[i-1] + 2 if s[i-1]==')' && s[i-2]==')' && s[i - dp[i-1]-1]=='('
    /// pub fn longest_valid_parentheses(s: String) -> i32 {
    ///     let s = s.as_bytes();
    ///     let n = s.len();
    ///     if n < 2 {
    ...
    ///             } else if i - 1 > dp[i - 1] {
    ///                 // symmetric position
    ///                 let sym_idx = i - dp[i - 1] - 2;
    ///                 if s[sym_idx] == LEFT {
    ///                     dp[i] = dp[i - 1] + 2 + if sym_idx > 0 { dp[sym_idx - 1] } else { 0 };
    ///                 }
    ///             }
    ///             res = res.max(dp[i]);
    ///         }
    ...
    ///
    /// date=20210824, mem=2.1, mem_beats=60, runtime=0, runtime_beats=100, url=https://leetcode-cn.com/submissions/detail/210655789/
    pub struct Solution;
...
pub mod solution_stack {
    ...
    ///
    /// date=20210824, mem=2, mem_beats=91, runtime=0, runtime_beats=100, url=https://leetcode-cn.com/submissions/detail/210659906/
    pub struct Solution;
